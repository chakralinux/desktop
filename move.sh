...
lock_repo()
{
	...
	lock=$(curl --silent --data "r=$(echo "${1}" | sed 's/ /+/g')&a=${_arch}&u=${_up}&n=${locker}" http://chakra-project.org/packages/add-lock.php)
	if [ "$lock" = "ok" ] ; then
		status_ok
	else
		status_fail
...
unlock_repo()
{
	...
	unlock=$(curl --silent --data "r=$(echo "${1}" | sed 's/ /+/g')&a=${_arch}&u=${_up}" http://chakra-project.org/packages/remove-lock.php)
	if [ "$unlock" = "ok" ] ; then
		status_ok
	else
		status_fail
...
move_and_recreate_db()
{
	...
	for pkg in $move_list ; do
		...
		unset moval
		moval=$(curl --silent --data "rs=${_cur}&rt=${final_dest}&a=${_arch}&u=${_up}&n=${pkg}" http://chakra-project.org/packages/move-pkg.php)
		if [ "$moval" = "ok" ] ; then
			status_done
...
move_and_recreate_db_smart()
{
	...
	for pkgs in $move_list ; do
		...
		moval=$(curl --silent --data "rs=${_cur}&rt=$(echo $pkgs | cut -d '|' -f2)&a=${_arch}&u=${_up}&n=$(echo $pkgs | cut -d '|' -f1)" http://chakra-project.org/packages/move-pkg.php)
		if [ "$moval" = "ok" ] ; then
			status_done
			mv -f ${_sync_folder}/$pkg _temp &>/dev/null
		else
...
if [ "${_args}" = "" ] ; then
	error " !! You need to specify a target to move and optionally a target repo,"
	error "    single names like «attica» or simple regexp like ^kde are allowed."
	error "    syntax: move.sh <pattern> <target repo>"
	newline
	exit 1
fi
