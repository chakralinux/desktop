...
lock_repo()
{
	...
	lock=$(curl --silent --data "r=${final_dest}&a=${_arch}&u=${_up}&n=${locker}" http://chakra-project.org/packages/add-lock.php)
	if [ "$lock" = "ok" ] ; then
		status_ok
	else
		status_fail
...
unlock_repo()
{
	...
	unlock=$(curl --silent --data "r=${final_dest}&a=${_arch}&u=${_up}" http://chakra-project.org/packages/remove-lock.php)
	if [ "$unlock" = "ok" ] ; then
		status_ok
	else
		status_fail
...
{
	newline
	status_start "Updating and cleaning database «$1-${_arch}»"
	recreate_db=$(curl --silent --data "r=$1&a=${_arch}&u=${_up}" http://chakra-project.org/packages/repo-update.php)
